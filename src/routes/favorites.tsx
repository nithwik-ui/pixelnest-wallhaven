...
function Favorites() {
  ...
  const filtered = useMemo(() => {
    let list = items;
    if (q) list = list.filter((w) => w.id.includes(q) || (w.tags ?? []).some((t) => t.name.toLowerCase().includes(q.toLowerCase())));
    ...
    <div className="mx-auto max-w-[1600px]">
        ...
        right={
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search favorites" className="h-10 rounded-full border border-border bg-[var(--color-surface)] pl-9 pr-4 text-sm outline-none focus:bg-background" />
            </div>
            <select value={sort} onChange={(e) => setSort(e.target.value as never)} className="h-10 rounded-full border border-border bg-[var(--color-surface)] px-4 text-sm outline-none">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-border bg-[var(--color-surface)] p-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">Tap the heart on any wallpaper to save it here.</p>
        </div>
      ) : (
